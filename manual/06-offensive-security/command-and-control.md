...
[Home](../../README.md) › [Offensive Security](README.md) › **Command and Control**
...
C2 architecture, channels, redirectors, malleable profiles and beacon detection. In one line: command and control decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, command and control comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Command and Control"] --> B["Concepts &amp; terminology"]
...
Uses the [Offensive Security lab environment](README.md#lab-environment).
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core command and control technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@command-and-control — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the command and control technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for command and control and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your command and control test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core command and control technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Command and Control address or create?**

<details><summary>Show answer</summary>

Command and Control matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Command and Control and what crosses it.**
...
**Q3. Give one attack against Command and Control and the single control that most reduces its impact.**
...
- Walk me through how Command and Control works, then tell me where you would attack it.
- How would you detect Command and Control being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Command and Control?

## Pitfalls & Best Practice

**Common mistakes**

- Treating command and control as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of command and control empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1071](https://attack.mitre.org/techniques/T1071/) |
| ATT&CK technique | [T1573](https://attack.mitre.org/techniques/T1573/) |
| ATT&CK technique | [T1090](https://attack.mitre.org/techniques/T1090/) |
| ATT&CK technique | [T1105](https://attack.mitre.org/techniques/T1105/) |
| ATT&CK technique | [T1568](https://attack.mitre.org/techniques/T1568/) |
...
_Part of the **Offensive Security** section of [Cybersecurity-Mastery](../../README.md). 🟠 Advanced · ~60 min hands-on · Last updated 2026-08-13._
