...
[Home](../../README.md) › [Offensive Security](README.md) › **Reconnaissance**
...
**Prerequisites:** [Offensive Security Methodology](offensive-methodology.md)

## Overview

Passive and active information gathering, footprinting and building a target model. In one line: reconnaissance decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, reconnaissance comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Reconnaissance"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core reconnaissance technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@recon — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the reconnaissance technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for reconnaissance and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your reconnaissance test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core reconnaissance technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Reconnaissance address or create?**

<details><summary>Show answer</summary>

Reconnaissance matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Reconnaissance and what crosses it.**
...
**Q3. Give one attack against Reconnaissance and the single control that most reduces its impact.**
...
- Walk me through how Reconnaissance works, then tell me where you would attack it.
- How would you detect Reconnaissance being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Reconnaissance?

## Pitfalls & Best Practice

**Common mistakes**

- Treating reconnaissance as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of reconnaissance empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1595](https://attack.mitre.org/techniques/T1595/) |
| ATT&CK technique | [T1592](https://attack.mitre.org/techniques/T1592/) |
| ATT&CK technique | [T1590](https://attack.mitre.org/techniques/T1590/) |
| ATT&CK technique | [T1589](https://attack.mitre.org/techniques/T1589/) |
