...
[Home](../../README.md) › [Offensive Security](README.md) › **Exfiltration**
...
Staging, compression, encryption and covert channels, plus DLP and egress detection. In one line: exfiltration decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, exfiltration comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Exfiltration"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core exfiltration technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@exfiltration — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the exfiltration technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for exfiltration and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your exfiltration test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core exfiltration technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Exfiltration address or create?**

<details><summary>Show answer</summary>

Exfiltration matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Exfiltration and what crosses it.**
...
**Q3. Give one attack against Exfiltration and the single control that most reduces its impact.**
...
- Walk me through how Exfiltration works, then tell me where you would attack it.
- How would you detect Exfiltration being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Exfiltration?

## Pitfalls & Best Practice

**Common mistakes**

- Treating exfiltration as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of exfiltration empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1041](https://attack.mitre.org/techniques/T1041/) |
| ATT&CK technique | [T1048](https://attack.mitre.org/techniques/T1048/) |
| ATT&CK technique | [T1567](https://attack.mitre.org/techniques/T1567/) |
| ATT&CK technique | [T1030](https://attack.mitre.org/techniques/T1030/) |
...
_Part of the **Offensive Security** section of [Cybersecurity-Mastery](../../README.md). 🟠 Advanced · ~48 min hands-on · Last updated 2026-08-13._
