...
[Home](../../README.md) › [Offensive Security](README.md) › **Defense Evasion**
...
Bypassing controls, obfuscation, and living-off-the-land, with the detections they trip. In one line: defense evasion decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, defense evasion comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Defense Evasion"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core defense evasion technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@defense-evasion — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the defense evasion technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for defense evasion and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your defense evasion test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core defense evasion technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Defense Evasion address or create?**

<details><summary>Show answer</summary>

Defense Evasion matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Defense Evasion and what crosses it.**
...
**Q3. Give one attack against Defense Evasion and the single control that most reduces its impact.**
...
- Walk me through how Defense Evasion works, then tell me where you would attack it.
- How would you detect Defense Evasion being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Defense Evasion?

## Pitfalls & Best Practice

**Common mistakes**

- Treating defense evasion as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of defense evasion empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1027](https://attack.mitre.org/techniques/T1027/) |
| ATT&CK technique | [T1070](https://attack.mitre.org/techniques/T1070/) |
| ATT&CK technique | [T1055](https://attack.mitre.org/techniques/T1055/) |
| ATT&CK technique | [T1218](https://attack.mitre.org/techniques/T1218/) |
| ATT&CK technique | [T1562](https://attack.mitre.org/techniques/T1562/) |
