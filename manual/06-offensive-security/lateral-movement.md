...
[Home](../../README.md) › [Offensive Security](README.md) › **Lateral Movement**
...
**Prerequisites:** [Privilege Escalation](privilege-escalation.md) · [Credential Access](credential-access.md) · [SMB](../01-networking/smb.md)

## Overview

Moving between hosts with remote execution, pivoting and relayed credentials. In one line: lateral movement decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, lateral movement comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Lateral Movement"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core lateral movement technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@lateral-movement — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the lateral movement technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for lateral movement and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your lateral movement test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core lateral movement technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Lateral Movement address or create?**

<details><summary>Show answer</summary>

Lateral Movement matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Lateral Movement and what crosses it.**
...
**Q3. Give one attack against Lateral Movement and the single control that most reduces its impact.**
...
- Walk me through how Lateral Movement works, then tell me where you would attack it.
- How would you detect Lateral Movement being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Lateral Movement?

## Pitfalls & Best Practice

**Common mistakes**

- Treating lateral movement as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of lateral movement empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1021](https://attack.mitre.org/techniques/T1021/) |
| ATT&CK technique | [T1570](https://attack.mitre.org/techniques/T1570/) |
| ATT&CK technique | [T1550](https://attack.mitre.org/techniques/T1550/) |
| ATT&CK technique | [T1563](https://attack.mitre.org/techniques/T1563/) |
