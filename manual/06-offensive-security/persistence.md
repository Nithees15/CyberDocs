...
[Home](../../README.md) › [Offensive Security](README.md) › **Persistence**
...
Maintaining access across reboots on Linux and Windows and the telemetry it generates. In one line: persistence decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, persistence comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Persistence"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core persistence technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@persistence — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the persistence technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for persistence and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your persistence test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core persistence technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Persistence address or create?**

<details><summary>Show answer</summary>

Persistence matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Persistence and what crosses it.**
...
**Q3. Give one attack against Persistence and the single control that most reduces its impact.**
...
- Walk me through how Persistence works, then tell me where you would attack it.
- How would you detect Persistence being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Persistence?

## Pitfalls & Best Practice

**Common mistakes**

- Treating persistence as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of persistence empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1543](https://attack.mitre.org/techniques/T1543/) |
| ATT&CK technique | [T1547](https://attack.mitre.org/techniques/T1547/) |
| ATT&CK technique | [T1053](https://attack.mitre.org/techniques/T1053/) |
| ATT&CK technique | [T1136](https://attack.mitre.org/techniques/T1136/) |
| ATT&CK technique | [T1505.003](https://attack.mitre.org/techniques/T1505/003/) |
