...
[Home](../../README.md) › [Networking](README.md) › **IPv6**
...
Addressing, SLAAC, NDP, extension headers, transition mechanisms and IPv6-only attack paths. In one line: IPv6 decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, IPv6 comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["IPv6"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core ipv6 technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@ipv6 — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the ipv6 technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for ipv6 and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your ipv6 test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core ipv6 technique on demand against the lab.
...
**Q1. In one sentence, what security problem does IPv6 address or create?**

<details><summary>Show answer</summary>

IPv6 matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in IPv6 and what crosses it.**
...
**Q3. Give one attack against IPv6 and the single control that most reduces its impact.**
...
- Walk me through how IPv6 works, then tell me where you would attack it.
- How would you detect IPv6 being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with IPv6?

## Pitfalls & Best Practice

**Common mistakes**

- Treating IPv6 as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of IPv6 empirically in the lab before trusting it in production.
...
- [RFC 4862](https://www.rfc-editor.org/rfc/rfc4862) — IPv6 SLAAC
