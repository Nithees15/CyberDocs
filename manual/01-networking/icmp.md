...
[Home](../../README.md) › [Networking](README.md) › **ICMP**

# ICMP

Message types, path MTU discovery, traceroute mechanics and ICMP covert channels.

<div class="meta-row"><span class="badge b-beginner">Beginner</span><span class="badge">⌨ ~30 min hands-on</span><span class="badge">📖 ~6 min read</span><button id="lesson-complete" class="lesson-check"><span class="lbl">Mark complete</span></button></div>
...
Message types, path MTU discovery, traceroute mechanics and ICMP covert channels. In one line: ICMP decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, ICMP comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["ICMP"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core icmp technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@icmp — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the icmp technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for icmp and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your icmp test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core icmp technique on demand against the lab.
...
**Q1. In one sentence, what security problem does ICMP address or create?**

<details><summary>Show answer</summary>

ICMP matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in ICMP and what crosses it.**
...
**Q3. Give one attack against ICMP and the single control that most reduces its impact.**
...
- Walk me through how ICMP works, then tell me where you would attack it.
- How would you detect ICMP being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with ICMP?

## Pitfalls & Best Practice

**Common mistakes**

- Treating ICMP as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of ICMP empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1095](https://attack.mitre.org/techniques/T1095/) |
| ATT&CK technique | [T1048](https://attack.mitre.org/techniques/T1048/) |
