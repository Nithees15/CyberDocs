...
[Home](../../README.md) › [Networking](README.md) › **LDAP**
...
Directory model, DNs, filters, binds, LDAPS/StartTLS, injection and enumeration. In one line: LDAP decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, LDAP comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["LDAP"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core ldap technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@ldap — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the ldap technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for ldap and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your ldap test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core ldap technique on demand against the lab.
...
**Q1. In one sentence, what security problem does LDAP address or create?**

<details><summary>Show answer</summary>

LDAP matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in LDAP and what crosses it.**
...
**Q3. Give one attack against LDAP and the single control that most reduces its impact.**
...
- Walk me through how LDAP works, then tell me where you would attack it.
- How would you detect LDAP being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with LDAP?

## Pitfalls & Best Practice

**Common mistakes**

- Treating LDAP as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of LDAP empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1087.002](https://attack.mitre.org/techniques/T1087/002/) |
| ATT&CK technique | [T1069.002](https://attack.mitre.org/techniques/T1069/002/) |
...
| CWE | Weakness |
| --- | --- |
| [CWE-90](https://cwe.mitre.org/data/definitions/90.html) | LDAP Injection |
...
- [RFC 4513](https://www.rfc-editor.org/rfc/rfc4513) — LDAP Authentication Methods
