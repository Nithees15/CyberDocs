...
[Home](../../README.md) › [Networking](README.md) › **Load Balancers and Proxies**
...
L4/L7 balancing, health checks, X-Forwarded-For trust, TLS termination and request smuggling. In one line: load balancers and proxies decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, load balancers and proxies comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Load Balancers and Proxies"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core load balancers and proxies technique step by step against metasploitable", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@load-balancers — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the load balancers and proxies technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for load balancers and proxies and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your load balancers and proxies test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core load balancers and proxies technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Load Balancers and Proxies address or create?**

<details><summary>Show answer</summary>

Load Balancers and Proxies matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Load Balancers and Proxies and what crosses it.**
...
**Q3. Give one attack against Load Balancers and Proxies and the single control that most reduces its impact.**
...
- Walk me through how Load Balancers and Proxies works, then tell me where you would attack it.
- How would you detect Load Balancers and Proxies being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Load Balancers and Proxies?

## Pitfalls & Best Practice

**Common mistakes**

- Treating load balancers and proxies as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of load balancers and proxies empirically in the lab before trusting it in production.
