...
[Home](../../README.md) › [Threat Intelligence and Frameworks](README.md) › **The Diamond Model and Kill Chains**

# The Diamond Model and Kill Chains
...
Adversary/infrastructure/capability/victim, kill chains and unified frameworks. In one line: the diamond model and kill chains decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, the diamond model and kill chains comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["The Diamond Model and Kill Chains"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core the diamond model and kill chains technique step by step against Kali / local shell", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@diamond-model — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the the diamond model and kill chains technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for the diamond model and kill chains and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your the diamond model and kill chains test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core the diamond model and kill chains technique on demand against the lab.
...
**Q1. In one sentence, what security problem does The Diamond Model and Kill Chains address or create?**

<details><summary>Show answer</summary>

The Diamond Model and Kill Chains matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in The Diamond Model and Kill Chains and what crosses it.**
...
**Q3. Give one attack against The Diamond Model and Kill Chains and the single control that most reduces its impact.**
...
- Walk me through how The Diamond Model and Kill Chains works, then tell me where you would attack it.
- How would you detect The Diamond Model and Kill Chains being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with The Diamond Model and Kill Chains?

## Pitfalls & Best Practice

**Common mistakes**

- Treating the diamond model and kill chains as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of the diamond model and kill chains empirically in the lab before trusting it in production.
