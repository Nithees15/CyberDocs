...
[Home](../../README.md) › [Threat Intelligence and Frameworks](README.md) › **Indicators: IOC and IOA**

# Indicators: IOC and IOA
...
Atomic/computed/behavioural indicators, the pyramid of pain and indicator decay. In one line: indicators: IOC and IOA decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, indicators: IOC and IOA comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Indicators: IOC and IOA"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core indicators: ioc and ioa technique step by step against Kali / local shell", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@ioc-ioa — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the indicators: ioc and ioa technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for indicators: ioc and ioa and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your indicators: ioc and ioa test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core indicators: ioc and ioa technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Indicators: IOC and IOA address or create?**

<details><summary>Show answer</summary>

Indicators: IOC and IOA matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Indicators: IOC and IOA and what crosses it.**
...
**Q3. Give one attack against Indicators: IOC and IOA and the single control that most reduces its impact.**
...
- Walk me through how Indicators: IOC and IOA works, then tell me where you would attack it.
- How would you detect Indicators: IOC and IOA being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Indicators: IOC and IOA?

## Pitfalls & Best Practice

**Common mistakes**

- Treating indicators: IOC and IOA as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of indicators: IOC and IOA empirically in the lab before trusting it in production.
