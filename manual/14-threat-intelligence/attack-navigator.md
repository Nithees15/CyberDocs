...
[Home](../../README.md) › [Threat Intelligence and Frameworks](README.md) › **ATT&CK Navigator**
...
Building layers, heatmaps, coverage mapping and gap analysis. In one line: ATT&CK navigator decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, ATT&CK navigator comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["ATT&CK Navigator"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core att&ck navigator technique step by step against Kali / local shell", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@attack-navigator — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the att&ck navigator technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for att&ck navigator and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your att&ck navigator test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core att&ck navigator technique on demand against the lab.
...
**Q1. In one sentence, what security problem does ATT&CK Navigator address or create?**

<details><summary>Show answer</summary>

ATT&CK Navigator matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in ATT&CK Navigator and what crosses it.**
...
**Q3. Give one attack against ATT&CK Navigator and the single control that most reduces its impact.**
...
- Walk me through how ATT&CK Navigator works, then tell me where you would attack it.
- How would you detect ATT&CK Navigator being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with ATT&CK Navigator?

## Pitfalls & Best Practice

**Common mistakes**

- Treating ATT&CK navigator as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of ATT&CK navigator empirically in the lab before trusting it in production.
