...
[Home](../../README.md) › [Web Security](README.md) › **Content Security Policy**
...
Directives, nonces, strict-dynamic, common bypasses and rollout with report-only. In one line: content security policy decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, content security policy comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Content Security Policy"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core content security policy technique step by step against bWAPP", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@csp — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the content security policy technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for content security policy and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your content security policy test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core content security policy technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Content Security Policy address or create?**

<details><summary>Show answer</summary>

Content Security Policy matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Content Security Policy and what crosses it.**
...
**Q3. Give one attack against Content Security Policy and the single control that most reduces its impact.**
...
- Walk me through how Content Security Policy works, then tell me where you would attack it.
- How would you detect Content Security Policy being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Content Security Policy?

## Pitfalls & Best Practice

**Common mistakes**

- Treating content security policy as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of content security policy empirically in the lab before trusting it in production.
...
| CWE | Weakness |
| --- | --- |
| [CWE-1021](https://cwe.mitre.org/data/definitions/1021.html) | Improper Restriction of Rendered UI Layers (Clickjacking) |
