...
[Home](../../README.md) › [Identity and Access](README.md) › **Active Directory**

# Active Directory

Forests, domains, trusts, GPOs, the schema and the objects attackers pivot through.

<div class="meta-row"><span class="badge b-advanced">Advanced</span><span class="badge">⌨ ~96 min hands-on</span><span class="badge">📖 ~32 min read</span><button id="lesson-complete" class="lesson-check"><span class="lbl">Mark complete</span></button></div>

**▶ Here to build, not read? Jump to the [Hands-On Practice](#hands-on-practice).**

**Prerequisites:** [Identity Fundamentals](identity-fundamentals.md)

## Overview

Forests, domains, trusts, GPOs, the schema and the objects attackers pivot through. In one line: active directory decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, active directory comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Active Directory"] --> B["Concepts &amp; terminology"]
...
> [!TIP] This is the main event. Bring up the lab, then work the walkthrough — every command has a **▸ Run** button (needs the lab runner) and a **📋 Copy** button. ~96 min, Kali attacker, fully local.

```cf-lab
{"title": "Identity and Access", "section": "12-identity", "targets": [["samba-ad", "LDAP/Kerberos/SMB directory services to enumerate and attack"], ["GOAD (optional)", "full multi-DC Windows forest for realistic AD attacks"]], "compose": "# id-lab/docker-compose.yml  —  Identity part environment\nservices:\n  samba-ad:\n    image: nowsci/samba-domain            # or diladele/samba-ad-dc\n    environment:\n      - DOMAIN=LAB.LOCAL\n      - DOMAINPASS=Passw0rd!Passw0rd!\n      - HOSTIP=172.20.0.10\n    networks:\n      labnet: { ipv4_address: 172.20.0.10 }\n    cap_add: [\"SYS_ADMIN\"]\n\nnetworks:\n  labnet:\n    driver: bridge\n    ipam: { config: [{ subnet: 172.20.0.0/24 }] }"}
```

Uses the [Identity and Access lab environment](README.md#lab-environment).
...
```cf-step
{"n": 1, "goal": "Provision & baseline", "cmd": "# bring up the part environment, then confirm you can reach `samba-ad` from Kali", "output": "Target reachable; you have a clean starting point.", "why": "Always capture 'normal' before you change anything — it's your reference.", "runnable": false}
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core active directory technique step by step against samba-ad", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@active-directory — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the active directory technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for active directory and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your active directory test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core active directory technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Active Directory address or create?**

<details><summary>Show answer</summary>

Active Directory matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Active Directory and what crosses it.**
...
**Q3. Give one attack against Active Directory and the single control that most reduces its impact.**
...
- Walk me through how Active Directory works, then tell me where you would attack it.
- How would you detect Active Directory being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Active Directory?

## Pitfalls & Best Practice

**Common mistakes**

- Treating active directory as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of active directory empirically in the lab before trusting it in production.
...
- [Identity Fundamentals](identity-fundamentals.md)
- [Entra ID (Azure AD)](azure-ad.md)
- [LDAP as an Identity Store](ldap-identity.md)
- [Kerberos in Depth](kerberos-identity.md)
- [NTLM](ntlm.md)
- [Single Sign-On](sso.md)

---

_Part of the **Identity and Access** section of [Cybersecurity-Mastery](../../README.md). 🟠 Advanced · ~96 min hands-on · Last updated 2026-08-13._
