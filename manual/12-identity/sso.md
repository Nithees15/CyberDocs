...
[Home](../../README.md) › [Identity and Access](README.md) › **Single Sign-On**
...
Federation patterns, SAML/OIDC trade-offs, session risks and IdP compromise impact. In one line: single sign-on decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, single sign-on comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Single Sign-On"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core single sign-on technique step by step against samba-ad", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@sso — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the single sign-on technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for single sign-on and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your single sign-on test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core single sign-on technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Single Sign-On address or create?**

<details><summary>Show answer</summary>

Single Sign-On matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Single Sign-On and what crosses it.**
...
**Q3. Give one attack against Single Sign-On and the single control that most reduces its impact.**
...
- Walk me through how Single Sign-On works, then tell me where you would attack it.
- How would you detect Single Sign-On being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Single Sign-On?

## Pitfalls & Best Practice

**Common mistakes**

- Treating single sign-on as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of single sign-on empirically in the lab before trusting it in production.
