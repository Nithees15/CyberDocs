...
[Home](../../README.md) › [Identity and Access](README.md) › **Passwordless and FIDO2**
...
WebAuthn ceremonies, passkeys, attestation and enterprise rollout. In one line: passwordless and FIDO2 decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, passwordless and FIDO2 comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Passwordless and FIDO2"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core passwordless and fido2 technique step by step against samba-ad", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@passwordless — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the passwordless and fido2 technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for passwordless and fido2 and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your passwordless and fido2 test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core passwordless and fido2 technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Passwordless and FIDO2 address or create?**

<details><summary>Show answer</summary>

Passwordless and FIDO2 matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Passwordless and FIDO2 and what crosses it.**
...
**Q3. Give one attack against Passwordless and FIDO2 and the single control that most reduces its impact.**
...
- Walk me through how Passwordless and FIDO2 works, then tell me where you would attack it.
- How would you detect Passwordless and FIDO2 being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Passwordless and FIDO2?

## Pitfalls & Best Practice

**Common mistakes**

- Treating passwordless and FIDO2 as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of passwordless and FIDO2 empirically in the lab before trusting it in production.
