...
[Home](../../README.md) › [Cryptography](README.md) › **Message Authentication Codes**

# Message Authentication Codes

MAC goals, CBC-MAC, CMAC, Poly1305 and encrypt-then-MAC composition rules.

<div class="meta-row"><span class="badge b-intermediate">Intermediate</span><span class="badge">⌨ ~30 min hands-on</span><span class="badge">📖 ~10 min read</span><button id="lesson-complete" class="lesson-check"><span class="lbl">Mark complete</span></button></div>

**▶ Here to build, not read? Jump to the [Hands-On Practice](#hands-on-practice).**

**Prerequisites:** [Cryptography Foundations](cryptography-foundations.md)

## Overview

MAC goals, CBC-MAC, CMAC, Poly1305 and encrypt-then-MAC composition rules. In one line: message authentication codes decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, message authentication codes comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Message Authentication Codes"] --> B["Concepts &amp; terminology"]
...
```cf-lab
{"title": "Cryptography", "section": "02-cryptography", "targets": [["Kali / local shell", "everything you need is a terminal and the tools named per chapter"]], "compose": "# Optional scratch box for trying commands in isolation:\nservices:\n  scratch:\n    image: kalilinux/kali-rolling        # or debian:stable-slim\n    container_name: scratch\n    command: sleep infinity\n    networks: [labnet]\nnetworks:\n  labnet: { driver: bridge, internal: true }"}
```

Uses the [Cryptography lab environment](README.md#lab-environment).
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core message authentication codes technique step by step against Kali / local shell", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@mac — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the message authentication codes technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for message authentication codes and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your message authentication codes test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core message authentication codes technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Message Authentication Codes address or create?**

<details><summary>Show answer</summary>

Message Authentication Codes matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Message Authentication Codes and what crosses it.**
...
**Q3. Give one attack against Message Authentication Codes and the single control that most reduces its impact.**
...
- Walk me through how Message Authentication Codes works, then tell me where you would attack it.
- How would you detect Message Authentication Codes being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Message Authentication Codes?

## Pitfalls & Best Practice

**Common mistakes**

- Treating message authentication codes as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of message authentication codes empirically in the lab before trusting it in production.
...
_Part of the **Cryptography** section of [Cybersecurity-Mastery](../../README.md). 🟡 Intermediate · ~30 min hands-on · Last updated 2026-08-13._
