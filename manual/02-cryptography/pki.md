...
[Home](../../README.md) › [Cryptography](README.md) › **Public Key Infrastructure**
...
CAs, RAs, trust stores, enrolment, internal PKI design and AD CS abuse paths. In one line: public key infrastructure decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, public key infrastructure comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Public Key Infrastructure"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core public key infrastructure technique step by step against Kali / local shell", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@pki — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the public key infrastructure technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for public key infrastructure and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your public key infrastructure test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core public key infrastructure technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Public Key Infrastructure address or create?**

<details><summary>Show answer</summary>

Public Key Infrastructure matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Public Key Infrastructure and what crosses it.**
...
**Q3. Give one attack against Public Key Infrastructure and the single control that most reduces its impact.**
...
- Walk me through how Public Key Infrastructure works, then tell me where you would attack it.
- How would you detect Public Key Infrastructure being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Public Key Infrastructure?

## Pitfalls & Best Practice

**Common mistakes**

- Treating public key infrastructure as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of public key infrastructure empirically in the lab before trusting it in production.
...
| Technique | ID |
| --- | --- |
| ATT&CK technique | [T1649](https://attack.mitre.org/techniques/T1649/) |
...
- [RFC 5280](https://www.rfc-editor.org/rfc/rfc5280) — PKIX Certificate Profile
...
_Part of the **Cryptography** section of [Cybersecurity-Mastery](../../README.md). 🟠 Advanced · ~60 min hands-on · Last updated 2026-08-13._
