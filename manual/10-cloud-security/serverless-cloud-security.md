...
[Home](../../README.md) › [Cloud Security](README.md) › **Serverless and PaaS Security**
...
Function isolation, event-source trust, secrets and cold-path telemetry gaps. In one line: serverless and PaaS security decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, serverless and PaaS security comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Serverless and PaaS Security"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core serverless and paas security technique step by step against LocalStack", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@serverless-cloud-security — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the serverless and paas security technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for serverless and paas security and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your serverless and paas security test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core serverless and paas security technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Serverless and PaaS Security address or create?**

<details><summary>Show answer</summary>

Serverless and PaaS Security matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Serverless and PaaS Security and what crosses it.**
...
**Q3. Give one attack against Serverless and PaaS Security and the single control that most reduces its impact.**
...
- Walk me through how Serverless and PaaS Security works, then tell me where you would attack it.
- How would you detect Serverless and PaaS Security being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Serverless and PaaS Security?

## Pitfalls & Best Practice

**Common mistakes**

- Treating serverless and PaaS security as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of serverless and PaaS security empirically in the lab before trusting it in production.
