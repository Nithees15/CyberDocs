...
[Home](../../README.md) › [Vulnerabilities](README.md) › **Local File Inclusion**
...
Include semantics, wrappers, log poisoning, session-file abuse and LFI-to-RCE chains. In one line: local file inclusion decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, local file inclusion comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Local File Inclusion"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core local file inclusion technique step by step against bWAPP / DVWA / Juice Shop", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@lfi — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the local file inclusion technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for local file inclusion and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your local file inclusion test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core local file inclusion technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Local File Inclusion address or create?**

<details><summary>Show answer</summary>

Local File Inclusion matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Local File Inclusion and what crosses it.**
...
**Q3. Give one attack against Local File Inclusion and the single control that most reduces its impact.**
...
- Walk me through how Local File Inclusion works, then tell me where you would attack it.
- How would you detect Local File Inclusion being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Local File Inclusion?

## Pitfalls & Best Practice

**Common mistakes**

- Treating local file inclusion as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of local file inclusion empirically in the lab before trusting it in production.
...
| CWE | Weakness |
| --- | --- |
| [CWE-98](https://cwe.mitre.org/data/definitions/98.html) | PHP Remote File Inclusion |
