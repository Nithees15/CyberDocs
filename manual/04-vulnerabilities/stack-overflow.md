...
[Home](../../README.md) › [Vulnerabilities](README.md) › **Stack-Based Overflows**
...
**Prerequisites:** [Buffer Overflows](buffer-overflow.md) · [Assembly Language](../00-foundations/assembly.md)

## Overview

Saved return addresses, canaries, ASLR/DEP, ROP chains and a full local lab exploit. In one line: stack-based overflows decides where trust is placed, and misplaced trust is where the security problem lives. That's the theory you need — the understanding comes from doing it, so the walkthrough below is the heart of this chapter.

### How it works

Mechanically, stack-based overflows comes down to three things: what data is trusted, where it crosses a boundary, and what happens on the other side. The diagram traces that path; you'll walk it for real, step by step, in the practice below.

```mermaid
flowchart TD
    A["Stack-Based Overflows"] --> B["Concepts &amp; terminology"]
...
```
```cf-step
{"n": 2, "goal": "Reproduce the technique", "cmd": "# perform the core stack-based overflows technique step by step against bWAPP / DVWA / Juice Shop", "output": "You observe the expected effect and can repeat it reliably.", "why": "Owning the mechanism by hand beats running a tool you don't understand.", "runnable": false}
```
```cf-step
...
```cf-terminal
{"title": "kali@stack-overflow — start the lab runner for a live shell"}
```
...
**Challenge 1.** Extend the walkthrough with one variation of the stack-based overflows technique and predict the result before running it.
...
**Challenge 2.** Find and apply a second defensive control for stack-based overflows and show it also blocks the technique.
...
**Challenge 3.** Write a one-paragraph report of your stack-based overflows test mapped to CWE/ATT&CK and the fix.
...
- [ ] Reproduce the core stack-based overflows technique on demand against the lab.
...
**Q1. In one sentence, what security problem does Stack-Based Overflows address or create?**

<details><summary>Show answer</summary>

Stack-Based Overflows matters because its behaviour determines where trust is placed; misplacing that trust is the root of the associated bug class.

</details>

**Q2. Name one trust boundary involved in Stack-Based Overflows and what crosses it.**
...
**Q3. Give one attack against Stack-Based Overflows and the single control that most reduces its impact.**
...
- Walk me through how Stack-Based Overflows works, then tell me where you would attack it.
- How would you detect Stack-Based Overflows being abused in an environment you defend?
- What is the difference between mitigating and eliminating the risk associated with Stack-Based Overflows?

## Pitfalls & Best Practice

**Common mistakes**

- Treating stack-based overflows as a checkbox instead of understanding the mechanism — defences copied without comprehension fail silently.
...
- Validate your understanding of stack-based overflows empirically in the lab before trusting it in production.
